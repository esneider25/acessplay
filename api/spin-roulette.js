...
export default async function handler(req, res) {
  ...
  const now = Date.now();
  if (!global.rateLimitRoulette) global.rateLimitRoulette = new Map();
  const rateLimit = global.rateLimitRoulette;
  ...
  try {
    ...
    if (isWinner) {
      ...
      if (product && product.packages && product.packages.length > 0) {
        ...
        const prizeOrderId = 'AP-' + Math.floor(10000 + Math.random() * 90000);
        const freeOrder = {
          id: prizeOrderId,
