// ============================================================
// AccessPlay — Dynamic Components (Redesigned + Orders)
// ============================================================
...
function renderProductDetail(productId) {
  ...
  if (product.isOutofStock) {
    ...
    packages = (product.packages || []).map((pkg, i) => {
      ...
      const priceHtml = (displayPriceUsd !== undefined && displayPriceUsd !== null) 
        ? `<div class="package-price-badge" data-usd="${displayPriceUsd.toFixed(2)}" data-bs="${formatBs(usdToBs(displayPriceUsd))}">Bs. ${formatBs(usdToBs(displayPriceUsd))}</div>`
        : '';
            ...
            <div class="tracking-rectify-container" style="margin-top: 20px; background: rgba(255,167,38,0.08); border: 1px solid rgba(255,167,38,0.25); border-radius: var(--radius-md); padding: 20px;">
              ...
              <div style="display: flex; flex-direction: column; gap: 12px;">
                ${order.productType === 'account' ? `
                  <input type="text" id="rectify-email-input-${order.id}" placeholder="Correo de la cuenta" style="width: 100%; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,167,38,0.4); border-radius: 8px; padding: 14px 16px; color: #fff; font-size: 1rem; outline: none; transition: border-color 0.3s;" onfocus="this.style.borderColor='#ffa726'" onblur="this.style.borderColor='rgba(255,167,38,0.4)'">
                  <input type="text" id="rectify-pass-input-${order.id}" placeholder="Contraseña de la cuenta" style="width: 100%; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,167,38,0.4); border-radius: 8px; padding: 14px 16px; color: #fff; font-size: 1rem; outline: none; transition: border-color 0.3s;" onfocus="this.style.borderColor='#ffa726'" onblur="this.style.borderColor='rgba(255,167,38,0.4)'">
                ` : order.productType === 'game-id-zone' ? `
                  <div style="display: flex; gap: 10px;">
                    <input type="text" id="rectify-id-input-${order.id}" placeholder="ID del Jugador" style="flex: 2; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,167,38,0.4); border-radius: 8px; padding: 14px 16px; color: #fff; font-size: 1rem; outline: none; transition: border-color 0.3s;" onfocus="this.style.borderColor='#ffa726'" onblur="this.style.borderColor='rgba(255,167,38,0.4)'">
                    <input type="text" id="rectify-zone-input-${order.id}" placeholder="Zona" style="flex: 1; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,167,38,0.4); border-radius: 8px; padding: 14px 16px; color: #fff; font-size: 1rem; outline: none; transition: border-color 0.3s;" onfocus="this.style.borderColor='#ffa726'" onblur="this.style.borderColor='rgba(255,167,38,0.4)'">
                  </div>
                ` : `
                  <input type="text" id="rectify-id-input-${order.id}" placeholder="Escribe aquí los datos correctos (ID)..." style="width: 100%; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,167,38,0.4); border-radius: 8px; padding: 14px 16px; color: #fff; font-size: 1rem; outline: none; transition: border-color 0.3s;" onfocus="this.style.borderColor='#ffa726'" onblur="this.style.borderColor='rgba(255,167,38,0.4)'">
                `}
    ...
    <section class="order-tracking-section" id="order-lookup">
      <div class="container">
        ...
        <div class="tracking-lookup-container">
          ...
          <h2>Mis Pedidos</h2>
          <p>Ingresa tu número de referencia (Ej: AP-1234) para ver el estado de tu pedido.</p>
          <div class="tracking-lookup-form">
            <input type="text" class="form-input" id="lookup-input"
                   placeholder="Ej: AP-1234" autocomplete="off"
...
function renderFeaturesSection() {
  return `
    <section class="features-section" id="features">
      <div class="container">
        <div class="section-header">
          <h2>¿Por qué <span class="gradient-text">AccessPlay</span>?</h2>
        ...
        <div class="footer-bottom">
          <p class="footer-disclaimer">
            ${(typeof LANDING_CONFIG !== 'undefined' && LANDING_CONFIG.footer && LANDING_CONFIG.footer.disclaimer) 
              ...
              : 'AccessPlay no está afiliado con Garena, Tencent, Roblox Corporation, miHoYo ni ninguna otra empresa mencionada.<br>Todos los nombres y logotipos son marcas registradas de sus respectivos dueños.'}
          </p>
          <p class="footer-copy">© ${new Date().getFullYear()} AccessPlay. Todos los derechos reservados.</p>
        </div>
      </div>
    </footer>
  `;
...
function renderSupportWidget() {
  ...
  return `
    <div class="support-widget" id="support-widget">
      <div class="support-options-menu" id="support-options-menu">
        <a href="${channelLink}" target="_blank" class="support-option-btn whatsapp-option">
          <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
          Canal de WhatsApp
        </a>
        <button class="support-option-btn web-option" onclick="openSupportWebChat()">
      ...
      <button class="support-fab" id="support-fab" onclick="toggleSupportChat()" aria-label="Ayuda">
        <span class="support-fab-text">Ayuda</span>
      ...
      <div class="support-chat" id="support-chat">
        <div class="support-chat-header">
          ...
          <button class="support-chat-close" onclick="closeSupportWebChat()">✕</button>
        </div>
        ...
        <div id="support-chat-bottom" style="display: none;">
          ...
          <div class="support-chat-input">
            <button class="support-menu-btn" onclick="closeSupportWebChat()" style="background:transparent; border:none; color:var(--text-secondary); cursor:pointer; font-size:1.2rem; padding:0 5px;" title="Volver al Menú">
              🔙
            </button>
