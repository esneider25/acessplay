...
mig_path = "migration.py"
with open(mig_path, "r", encoding="utf-8") as f:
    mig_content = f.read()

mig_content = mig_content.replace('f"RS-OLD-{p[\'id\']}"', 'f"AP-OLD-{p[\'id\']}"')
mig_content = mig_content.replace('"role": "reseller" if u[\'rol\'] == \'reseller\' else ("admin" if u[\'rol\'] == \'admin\' else "user"),', '"role": "revendedor" if u[\'rol\'] == \'reseller\' else ("admin" if u[\'rol\'] == \'admin\' else "cliente"),')
