...
window.adminTournamentLimit = window.adminTournamentLimit || 5;
  ...
  let html = `
    ...
    <div class="admin-card">
      ...
      <div style="text-align: center; margin-top: 20px;">
        <button id="btn-load-more-admin-tournaments" class="btn btn-secondary" onclick="window.adminTournamentLimit += 5; renderTournaments(document.getElementById('content-area'))" style="display: none; padding: 10px 20px;">Cargar más torneos</button>
      </div>
    </div>
  `;
  container.innerHTML = html;

  // Cargar metadatos para las estadísticas globales
  firebase.database().ref('tournament_metadata').on('value', metaSnap => {
    const statsBar = document.getElementById('tournament-stats-bar');
    if (!statsBar) return;
    const meta = metaSnap.val() || { active: 0, completed: 0, participants: 0 };
    // Para el total, asumiendo active + completed + upcoming. Vamos a cargar también el total de nodos si es necesario, o lo leemos del meta.
    // Como el meta no guardó "total", podemos mostrar solo los que sí tenemos.
    statsBar.innerHTML = `
      <div style="background:var(--bg-deep); border:1px solid var(--border); border-radius:var(--radius-sm); padding:16px; text-align:center;">
        <div style="font-size:1.6rem; font-weight:800; color:#38bdf8; font-family:var(--font-display);">${meta.active || 0}</div>
        <div style="font-size:0.75rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.5px;">Activos</div>
      </div>
      <div style="background:var(--bg-deep); border:1px solid var(--border); border-radius:var(--radius-sm); padding:16px; text-align:center;">
        <div style="font-size:1.6rem; font-weight:800; color:#4ade80; font-family:var(--font-display);">${meta.completed || 0}</div>
        <div style="font-size:0.75rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.5px;">Completados</div>
      </div>
      <div style="background:var(--bg-deep); border:1px solid var(--border); border-radius:var(--radius-sm); padding:16px; text-align:center;">
        <div style="font-size:1.6rem; font-weight:800; color:#fbbf24; font-family:var(--font-display);">${meta.participants || 0}</div>
        <div style="font-size:0.75rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.5px;">Participantes Históricos</div>
      </div>
    `;
  });

  firebase.database().ref('tournaments').orderByChild('createdAt').limitToLast(window.adminTournamentLimit).on('value', snapshot => {
    const listContainer = document.getElementById('tournaments-list');
    const loadMoreBtn = document.getElementById('btn-load-more-admin-tournaments');
    ...
    try {
      ...
      if (loadMoreBtn) {
        if (torneos.length >= window.adminTournamentLimit) {
      ...
      torneos.forEach(torneo => {
        ...
        if (torneo.registrationDeadline) {
          try { deadlineStr = ' | Cierre: ' + new Date(torneo.registrationDeadline).toLocaleString([], { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }); } catch(e) {}
        }
      ...
      if (p) {
        ...
        firebase.database().ref('tournament_participants/' + tournamentId + '/' + userId).remove().then(() => {
          if (p.paymentStatus === 'approved' || p.paymentStatus === 'free') {
             ...
             firebase.database().ref('tournament_metadata/participants').transaction(c => (c || 0) - countSub);
          }
...
window.exportParticipantsCSV = function(id) {
  Promise.all([
    ...
    const torneo = snaps[0].val();
    const partsSnap = snaps[1];
    const participants = Object.values(partsSnap.val() || {});
    ...
    if (groupedParticipants.length > 0 || leaderboard.length > 0) {
      ...
      groupedParticipants.forEach((p) => {
        ...
        if (teamType !== 'Solo') {
          ...
          leaderboardRows += `
            <tr style="background:rgba(255,255,255,0.05);">
              <td colspan="3" style="padding:8px 10px; font-size:0.75rem; color:var(--accent); font-weight:bold; text-transform:uppercase;">
                👥 Equipo ${teamType} - Líder: ${pName} ${teamPosInputHTML}
              </td>
            </tr>
          `;
        }
        ...
        const inputsHTML = (isPrizeFormat && teamType === 'Solo')
          ...
          : `<input type="number" class="admin-form-input bulk-kill-input" data-player="${pName.replace(/"/g, '&quot;')}" data-leader="${teamType !== 'Solo' ? pName.replace(/"/g, '&quot;') : ''}" value="${capKills}" min="0" style="width:70px; padding:6px; margin:0;" title="Kills logradas">`;
...
window.saveBulkLeaderboard = function(id) {
  ...
  const teamPosInputs = document.querySelectorAll('.bulk-team-pos-input');
  ...
  inputs.forEach(input => {
    ...
    const leaderName = input.getAttribute('data-leader');
    if (!playerStats[playerName]) playerStats[playerName] = { playerName, kills: 0, position: 0, leaderName: leaderName || null };
  ...
  teamPosInputs.forEach(input => {
    const leaderName = input.getAttribute('data-leader');
    const teamPos = parseInt(input.value) || 0;
    Object.values(playerStats).forEach(stat => {
      if (stat.leaderName === leaderName) {
        stat.position = teamPos;
      }
    });
  });
...
// ── Migración de Torneos a Nueva Estructura ──
window.migrateTournamentsData = async function() {
  const confirmacion = await Swal.fire({
    title: '¿Iniciar Migración de Torneos?',
    text: 'Este proceso moverá los participantes a la nueva estructura "tournament_participants" y creará los metadatos globales. ¡Haz esto solo si no hay usuarios escribiendo actualmente!',
    icon: 'warning',
  ...
  const btn = document.getElementById('btn-migrate-tournaments');
  if(btn) { btn.innerHTML = '<div class="spinner" style="width:15px; height:15px; border-width:2px; margin-right:10px;"></div> Migrando...'; btn.disabled = true; }
  ...
  try {
    ...
    for (const tId in tournaments) {
      ...
      if (status === 'registration_open' || status === 'ongoing') totalActive++;
      if (status === 'completed' || status === 'completado') totalCompleted++;
      ...
      if (t.participants) {
        updates['tournaments/' + tId + '/participants'] = null; 
      }
      if (t.participantsCount !== count) {
      updates['tournaments/' + tId + '/participantsCount'] = count;
      }
      
      // Calculate earnings and wins for historical
      const winners = t.winners || [];
      winners.forEach(w => {
         if (w.uid && playerStats[w.uid]) {
             playerStats[w.uid].totalWins += 1;
    ...
    const premiumPlayers = Object.values(playerStats).filter(p => p.type === 'premium');
    const freePlayers = Object.values(playerStats).filter(p => p.type === 'free');

    updates['tournament_global_stats'] = {
        premium: premiumPlayers,
        free: freePlayers
    };
    updates['tournament_metadata'] = {
        active: totalActive,
